...
Result_t TimedLong_set(DataType_t d, uint32_t sec, uint32_t nsec, long data) {
...
Result_t TimedLong_get(DataType_t d, uint32_t* sec, uint32_t* nsec, long* data) {
  if (d < 0 || d >= _data.size()) {return INVALID_DATA; }
  *sec = _data[d]->tm.sec;
...
Port_t InPort_TimedLong_create(char* name, DataType_t d) {
  ...
  //std::cout << "InPort_TimedLong_create(" << name << ", " << d << ")" << std::endl;
  //addPort(std::shared_ptr<RTC::PortBase>(new RTC::InPort<RTC::TimedLong>(name, *(_data[d]))));
  //addPort(new RTC::InPort<RTC::TimedLong>(name, *(_data[d])));
...
Port_t OutPort_TimedLong_create(char* name, DataType_t d) {
  ...
  //addPort(new RTC::OutPort<RTC::TimedLong>(name, *(_data[d])));
  return __ports->size() - 1;
}
